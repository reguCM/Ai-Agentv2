...
def build_initial_user(pytest_run, program_run):
    return (
        "Analyze the current problem and determine what should be done next.\n\n"
        "The program failed with the following output:\n\n"
...
def format_file_result(path, text):
    return f"You asked to inspect {path}.\n\nHere is the file:\n\n{text}\n"


def format_test_failure(pytest_run, program_run):
    return (
        "The test still fails.\n\n"
        f"Exit code:\n{pytest_run['exit_code']}\n\n"
        f"stdout:\n{pytest_run.get('stdout') or ''}\n"
        f"stderr:\n{pytest_run.get('stderr') or ''}\n"
        f"Traceback:\n{pytest_run.get('traceback') or program_run.get('traceback') or ''}\n\n"
        + format_execution_block("python main.py", program_run)
        + "\n"
    )
