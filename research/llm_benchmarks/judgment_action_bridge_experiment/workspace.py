"""実験専用 Tool。既存実験 workspace / 本番 Registry には接続しない。"""
...
FIXTURE_SRC = Path(__file__).resolve().parent / "fixtures" / "chain_count"
    ...
    for file_path in files:
        ...
        for index, line in enumerate(text.splitlines(), start=1):
            if str(query) in line:
                matches.append({"path": _rel(workspace, file_path), "line": index, "text": line})
