#!/usr/bin/env python3
"""Run Web Tool Success-Class Accuracy Evaluation."""
...
from ai_tool.web_tool_success_class_accuracy_evaluation import run_success_class_accuracy_evaluation
from tools.system.config import get_llm_profile

RUN_ID = datetime.now().strftime("%Y%m%d_%H%M%S") + "_web_tool_success_class_accuracy_evaluation"
...
def main() -> int:
    ...
    result = run_success_class_accuracy_evaluation(
        include_live=True,
        llm_enabled=ok,
        chat_fn=chat,
        model=model,
        ...
        json.dumps(
            {
                "run_id": RUN_ID,
                "overall": result.get("overall"),
                "accuracy": result.get("accuracy_rate"),
                "web_success": result.get("web_success_count"),
